...
const BillingGate = {
  ...
  // בדיקת מגבלת דוחות
  const limitCheck = BillingCore.checkReportLimit(this.token);
  const reportsUsed = limitCheck.used || 0;
  const reportsMax  = limitCheck.max  || 8;
  const reportsLeft = Math.max(0, reportsMax - reportsUsed);
  ...
  b.innerHTML = `
    <span>${urgency} נותרו <strong>${daysLeft} ימים</strong> בניסיון · <strong>${reportsLeft} דוחות</strong> מתוך ${reportsMax}</span>
    ...
    <button onclick="document.getElementById('omda-trial-banner').remove()" style="background:transparent;border:none;color:#fff;cursor:pointer;font-size:18px;">×</button>
  `;
